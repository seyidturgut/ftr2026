...
const PPTXViewer: React.FC<PPTXViewerProps> = ({ fileUrl, onDownload }) => {
    ...
    // Mock slides for the sidebar since we can't easily get them from PPTX
    // We'll show a placeholder list of slides
    const mockSlides = Array.from({ length: 10 }, (_, i) => i + 1);
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div className="flex items-center bg-gray-100 rounded-lg p-1 border border-gray-200">
                        <button
                            ...
                            className={`p-1.5 rounded transition-all flex items-center gap-2 px-3 ${sidebarOpen ? 'bg-white text-sky-600 shadow-sm' : 'text-gray-500 hover:text-gray-800'}`}
                        >
                            <Layout size={16} />
                            <span className="text-[10px] font-black uppercase whitespace-nowrap">Navigasyon</span>
                        </button>
                    </div>
                </div>
            ...
            <div className="flex flex-1 overflow-hidden">
                ...
                {sidebarOpen && (
                    <div className="w-24 md:w-56 bg-gray-50 border-r border-gray-200 overflow-y-auto flex-shrink-0 custom-scrollbar p-4 flex flex-col gap-4">
                        <div className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2 px-2">Slaytlar</div>
                        {mockSlides.map((num) => (
                            <div
                                key={`slide_${num}`}
                                className="group cursor-default"
                            >
                                <div className="aspect-video bg-white rounded-lg overflow-hidden border border-gray-200 shadow-sm flex flex-col items-center justify-center gap-2 group-hover:border-sky-300 transition-colors">
                                    <FileText size={20} className="text-gray-300 group-hover:text-sky-400" />
                                    <span className="text-[10px] font-black text-gray-400 group-hover:text-sky-600">SLAYT {num}</span>
                                </div>
                            </div>
                        ))}
                        <div className="p-4 bg-orange-50 rounded-xl border border-orange-100 mt-4">
                            <p className="text-[10px] text-orange-600 font-bold leading-tight">Navigasyon için sunum içindeki kontrolleri kullanın.</p>
                        </div>
                    </div>
                )}
