...
const Footer: React.FC = () => {
  return (
    <footer className="relative bg-slate-50 dark:bg-slate-950 pt-20 pb-10 overflow-hidden transition-colors duration-500">
      {/* Decorative top border with gradient fade */}
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-6 gap-12 mb-16">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <p className="text-slate-600 dark:text-slate-400 text-sm leading-relaxed max-w-sm">
              Rehabilitasyon uzmanlarını yeni nesil yapay zeka araçları ve veri odaklı içgörülerle güçlendiriyoruz. Terapinin geleceği akışkan, akıllı ve erişilebilir.
            </p>
          ...
          <div className="lg:col-span-1">
            ...
            <ul className="space-y-4 text-sm text-slate-600 dark:text-slate-400">
              {['Özellikler', 'YZ Analizi', 'Tele-Rehab', 'Güvenlik', 'Fiyatlandırma'].map(item => (
          ...
          <div className="lg:col-span-1">
            <h4 className="font-semibold text-slate-900 dark:text-white mb-6">Kaynaklar</h4>
            <ul className="space-y-4 text-sm text-slate-600 dark:text-slate-400">
              {['Dokümantasyon', 'API Referansı', 'Vaka Çalışmaları', 'Topluluk', 'Yardım Merkezi'].map(item => (
                <li key={item}><a href="#" className="hover:text-sky-600 dark:hover:text-sky-400 transition-colors block">{item}</a></li>
              ))}
            </ul>
          </div>
          ...
          <div className="lg:col-span-1">
            <h4 className="font-semibold text-slate-900 dark:text-white mb-6">Şirket</h4>
            <ul className="space-y-4 text-sm text-slate-600 dark:text-slate-400">
              {['Hakkımızda', 'Kariyer', 'Blog', 'İletişim', 'İş Ortakları'].map(item => (
                <li key={item}><a href="#" className="hover:text-sky-600 dark:hover:text-sky-400 transition-colors block">{item}</a></li>
              ))}
            </ul>
          </div>
        ...
        {/* Newsletter Strip */}
        <div className="relative rounded-2xl bg-white/60 dark:bg-white/5 border border-slate-200 dark:border-white/10 p-8 mb-12 flex flex-col md:flex-row items-center justify-between gap-8 overflow-hidden group">
          ...
          <div className="relative z-10 text-center md:text-left">
            <h4 className="text-xl font-semibold text-slate-900 dark:text-white mb-2">Bültenimize abone olun</h4>
            <p className="text-slate-600 dark:text-slate-400 text-sm">Platform güncellemeleri hakkında en son haberleri alın.</p>
          </div>
          ...
          <div className="relative z-10 w-full md:w-auto flex flex-col sm:flex-row gap-3">
            <div className="relative">
              ...
              <input 
                ...
                className="w-full sm:w-64 bg-white/80 dark:bg-slate-950/50 border border-slate-200 dark:border-white/10 rounded-xl pl-10 pr-4 py-3 text-slate-900 dark:text-white placeholder:text-slate-500 focus:outline-none focus:border-sky-500/50 focus:ring-1 focus:ring-sky-500/50 transition-all"
              />
            </div>
            <button className="px-6 py-3 bg-slate-900 dark:bg-white text-white dark:text-slate-950 font-bold rounded-xl hover:bg-slate-800 dark:hover:bg-sky-50 transition-colors shadow-lg shadow-slate-900/10 dark:shadow-white/5">
              Abone Ol
            </button>
          </div>
        </div>
        ...
        <div className="flex flex-col md:flex-row items-center justify-between text-xs text-slate-500 gap-6 border-t border-slate-200 dark:border-white/5 pt-8">
          ...
          <div className="flex gap-8">
            <a href="#" className="hover:text-slate-800 dark:hover:text-slate-300 transition-colors">Gizlilik Politikası</a>
            <a href="#" className="hover:text-slate-800 dark:hover:text-slate-300 transition-colors">Kullanım Şartları</a>
            <a href="#" className="hover:text-slate-800 dark:hover:text-slate-300 transition-colors">Çerez Ayarları</a>
          </div>
        </div>
      </div>
    </footer>
